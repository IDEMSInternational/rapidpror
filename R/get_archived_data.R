...
#' @export
get_archived_data <- function(rapidpro_site = get_rapidpro_site(), token = get_rapidpro_key(), period = "monthly", flatten = FALSE, date_from = NULL, date_to = NULL, format_date = "%Y-%m-%d", tzone_date = "UTC"){
  ...
  result_flow <- result_flow %>% dplyr::filter(archive_type == "run")
