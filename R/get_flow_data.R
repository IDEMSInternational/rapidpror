...
#' @param flow_type Default `none`. Takes values `"praise"`, `"calm"`, `"check_in"`, `"tips"`, `"none"`, `"other"`. These are related to ParentText.
                            ...
                            tzone_date = "UTC") 
{
  ...
  for (i in flow_name) {
    ...
    get_command <- paste(rapidpro_site, call_type, uuid_flow[1], 
                         sep = "")
    result_flow <- rapidpror:::httr_get_call(get_command = get_command, 
                                             token = token)
